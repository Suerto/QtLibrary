...
unordered_map<string, string> BookFilters::raccogliDati() const {
    ...
    qDebug() << "Avviata la raccolta dati di un libro";
    //Contenuto
    parametri.insert({"Anno", std::to_string(year->value())});
    qDebug() << "Anno Salvato :" << year->value();
    ...
    qDebug() << "Lingua Salvata : " << language->currentText(); 
    //Fisico
    ...
    qDebug() << "Publisher salvato : " << publisher->text();
    ...
    qDebug() << "Genere Salvato" << genre->currentText();
    
    Filters* test = new BookFilters;
...
void BookFilters::setModifiable(const bool& mdf) {
    ...
    author->setStyleSheet(!mdf ? "QLineEdit { border: none; background: transparent; }" : "");
    ...
    publisher->setStyleSheet(!mdf ? "QLineEdit { border: none; background: transparent; }" : "");
